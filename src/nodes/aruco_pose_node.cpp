#include <charuco_detect/nodes/aruco_pose_node.hpp>
...
{
  // convert everything
  auto cv_image = cv_bridge::toCvShare(image);
  ...
  auto marker_poses = aruco_pose(markers, camera_matrix, dist_coeffs,
                                          marker_length);
  // convert the poses
  ...
  for (size_t i = 0; i < marker_poses.size(); i++)
  {
    ...
    tf2_msg.child_frame_id = "aruco_marker_" + markers.ids[i];
